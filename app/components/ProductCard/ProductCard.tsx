      ...
      <div className="product-card-inner">
        ...
        <div className="product-details-col">
          ...
          <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            {
              product.expiration && 
                <div className="data-cell max-w-[350px]">
                  <span className="data-cell-label">Notes:</span>
                  <span className="data-cell-value flex justify-around gap-2 items-start bg-amber-200 rounded-lg py-2! px-4!">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-30 h-auto text-amber-500">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                    </svg>
                    <span className="text-md text-amber-500">Ce produit a une date d&apos;expiration; assurez-vous d&apos;effectuer correctement la rotation des stocks afin d&apos;expédier les produits selon leur date de péremption.</span>
                  </span>
                </div>
            }
          ...
          <div>
            ...
            </div>
            {
              (Array.isArray(product.bin_location) ? product.bin_location : product.bin_location?.split(',') || []).map((location:string, index: number) => (
                <span key={index} className={`modal-sku mr-2! ${binLocationValue === location.trim() && 'bg-green-100! border-green-700! '} hidden!`}>{location.trim()}</span>
              ))
            }
          </div>
        }
